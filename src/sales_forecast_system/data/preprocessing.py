...
class DataPreprocessor:
    ...
    def handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Handle markdown missing values (fill with 0 as in notebook)
        for col in self.markdown_cols:
            if col in df_copy.columns:
                df_copy[col] = df_copy[col].fillna(0)
        
        # Handle CPI and Unemployment using forward fill then backward fill
    ...
    def create_date_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create date features (matching notebook)
        ...
        # Create holiday indicator (matching notebook)
    ...
    def encode_categorical(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Type' in df_copy.columns:
            le = LabelEncoder()
            df_copy['Type_encoded'] = le.fit_transform(df_copy['Type'].astype(str))
            logger.info(f"Encoded Type: {dict(zip(le.classes_, le.transform(le.classes_)))}")
